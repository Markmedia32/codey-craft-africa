        ...
        <div className="container">
          {hasJobs ? (
                  ...
                  <div key={job.id} className="job-card-expanded" style={{ 
                    ...
                    <div className="job-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px' }}>
                      <div>
                        ...
                        <p style={{ fontSize: '1.1rem', color: '#555', maxWidth: '800px', lineHeight: '1.6' }}>{job.desc}</p>
                      </div>
                      <button 
                    ...
                    <div className="job-details-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', marginTop: '40px', paddingTop: '40px', borderTop: '1px solid #f0f0f0' }}>
                      <div>
                        <h4 style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}><FaCheckCircle color="var(--cca-red)"/> Requirements</h4>
                      ...
                      <div>
                        <h4 style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}><FaLayerGroup color="var(--cca-red)"/> Desired Skills</h4>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                          {job.skills && job.skills.map((skill, i) => (
                            <span key={i} style={{ background: '#f0f0f0', padding: '8px 16px', fontSize: '0.8rem', fontWeight: '700', borderRadius: '4px' }}>{skill.toUpperCase()}</span>
                          ))}
                        </div>
                      </div>
                    </div>
            ...
            <div style={{ textAlign: 'center', padding: '100px 0' }}>
              ...
              <p style={{ opacity: 0.5 }}>We aren't hiring right now, but we're always looking for talent. Check back soon!</p>
            </div>
          )}
        </div>
      </section>
